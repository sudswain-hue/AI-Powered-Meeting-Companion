...
def main():
    ...
    if uploaded_file is not None:
        ...
        # Analysis results in a row
        st.markdown("---")
        
        # Create three columns for the analysis sections
        ...
        with col1:
            ...
            with st.spinner("Generating summary..."):
                ...
                st.text_area("", summary_text, height=250)
                st.download_button(
        ...
        with col3:
            ...
            with st.spinner("Performing sentiment analysis..."):
                ...
                # Create scrollable container for detailed sentiment results
                sentiment_text = ""
                
                # Create a container with fixed height for scrolling
                with st.container():
                    st.markdown("#### Detailed Sentiment Analysis")
                    sentiment_details = st.empty()
                    
                    # Create the scrollable content using custom HTML
                    sentiment_html = "<div style='height: 150px; overflow-y: scroll;'>"
                    for idx, result in enumerate(sentiments, 1):
                        sentiment_html += f"<p><b>{idx}. Text:</b> {result['text']}<br>"
                        sentiment_html += f"<b>Sentiment:</b> {result['sentiment']} (Label: {result['label']}, Confidence: {result['score']:.2f})</p>"
                        sentiment_html += "<hr>"
                    ...
                    sentiment_html += "</div>"
                    sentiment_details.markdown(sentiment_html, unsafe_allow_html=True)
